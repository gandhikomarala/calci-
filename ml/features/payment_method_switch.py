"""PaymentMethodSwitchTransformer: Sudden shift in payment instrument type vs historical customer habits."""
...
logger = get_logger("payment_method_switch")

class PaymentMethodSwitchTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, output_col: str = "payment_method_switch_val", weight: float = 1.0):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> "PaymentMethodSwitchTransformer":
        logger.info("Fitting PaymentMethodSwitchTransformer", input_rows=len(X))
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        if not self.is_fitted_:
            raise RuntimeError("PaymentMethodSwitchTransformer must be fitted before calling transform()")
        
        X_out = X.copy()
        
        # Domain-specific financial fraud feature transformations
        if "payment_method_switch" == "velocity_5m" and "velocity_last_1h" in X_out.columns:
            v1h = X_out["velocity_last_1h"].fillna(0)
            X_out[self.output_col] = np.clip(v1h * 0.35, 0, 50) * self.weight
        elif "payment_method_switch" == "amount_deviation" and "amount" in X_out.columns:
        ...
        elif "payment_method_switch" == "location_anomaly" and "is_location_anomaly" in X_out.columns:
            loc = X_out["is_location_anomaly"].fillna(0)
            X_out[self.output_col] = loc * 2.5 * self.weight
        elif "payment_method_switch" == "device_risk_scorer" and "is_new_device" in X_out.columns:
        ...
        elif "payment_method_switch" == "failed_auth_velocity" and "failed_attempts_last_24h" in X_out.columns:
            fails = X_out["failed_attempts_last_24h"].fillna(0)
            X_out[self.output_col] = np.log1p(fails * 2.0) * self.weight
        elif "payment_method_switch" == "merchant_risk_index" and "merchant_category" in X_out.columns:
