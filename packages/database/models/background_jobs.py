"""SQLAlchemy 2.0 Mapped Entity Model for BackgroundJob (Asynchronous Celery task execution status and progress tracking)."""
...
class BackgroundJob(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_background_jobs_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<BackgroundJob(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
