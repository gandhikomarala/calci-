"""Domain Business Logic Service for RiskRule."""
...
logger = get_logger("risk_rules_service")

class RiskRuleService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = RiskRuleRepository(session)

    async def list_items(self, params: RiskRuleFilterParams) -> Tuple[Sequence[RiskRule], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing RiskRule items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> RiskRule:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("RiskRule not found", item_id=item_id)
            raise NotFoundError(f"RiskRule with ID '{item_id}' was not found")
        return entity

    async def create(self, req: RiskRuleCreate) -> RiskRule:
        entity = RiskRule()
        ...
        logger.info("RiskRule created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: RiskRuleUpdate) -> RiskRule:
        ...
        logger.info("RiskRule updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("RiskRule deleted successfully", item_id=item_id)
