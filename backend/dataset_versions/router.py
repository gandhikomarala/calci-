"""FastAPI REST API Router for DatasetVersion (Immutable versioned dataset snapshots with row counts and hashes)."""
...
router = APIRouter(prefix="/dataset_versions", tags=["DatasetVersion"])

@router.get("", response_model=PaginatedResponse[DatasetVersionResponse])
async def list_items(
    ...
    service = DatasetVersionService(session)
    params = DatasetVersionFilterParams(
        page=page,
        page_size=page_size,
        search=search,
        is_active=is_active,
...
@router.get("/{item_id}", response_model=DatasetVersionResponse)
...
async def create_item(req: DatasetVersionCreate, session: AsyncSession = Depends(get_db_session)):
    service = DatasetVersionService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=DatasetVersionResponse)
async def update_item(item_id: str, req: DatasetVersionUpdate, session: AsyncSession = Depends(get_db_session)):
    service = DatasetVersionService(session)
