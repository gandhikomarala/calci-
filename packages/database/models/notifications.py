"""SQLAlchemy 2.0 Mapped Entity Model for Notification (In-app and outgoing alert notifications for users)."""
...
class Notification(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_notifications_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<Notification(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
