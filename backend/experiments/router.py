"""FastAPI REST API Router for Experiment (ML experiment tracking namespaces and benchmark objectives)."""
...
@router.get("/{item_id}", response_model=ExperimentResponse)
async def get_by_id(item_id: str, session: AsyncSession = Depends(get_db_session)):
    service = ExperimentService(session)
    return await service.get_by_id(item_id)

@router.post("", response_model=ExperimentResponse, status_code=status.HTTP_201_CREATED)
async def create_item(req: ExperimentCreate, session: AsyncSession = Depends(get_db_session)):
    service = ExperimentService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=ExperimentResponse)
async def update_item(item_id: str, req: ExperimentUpdate, session: AsyncSession = Depends(get_db_session)):
    service = ExperimentService(session)
