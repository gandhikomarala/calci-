"""Pydantic v2 Schemas for InvestigationAssignment (Analyst case assignments, workload distribution, and SLAs)."""

from typing import Optional, List, Dict, Any
import datetime
from pydantic import BaseModel, Field, ConfigDict

class InvestigationAssignmentBase(BaseModel):
...
class InvestigationAssignmentCreate(InvestigationAssignmentBase):
    pass
...
class InvestigationAssignmentResponse(InvestigationAssignmentBase):
...
class InvestigationAssignmentDetailResponse(InvestigationAssignmentResponse):
    extra_attributes: Optional[Dict[str, Any]] = Field(default=None)
    audit_history: Optional[List[Dict[str, Any]]] = Field(default=None)

class InvestigationAssignmentFilterParams(BaseModel):
