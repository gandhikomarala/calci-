"""Unit test for Permission Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.permissions.service import PermissionService
from backend.permissions.schemas import PermissionCreate, PermissionFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_permissions_list_items():
    mock_session = AsyncMock()
    service = PermissionService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = PermissionFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_permissions_get_by_id():
    mock_session = AsyncMock()
    service = PermissionService(mock_session)
...
@pytest.mark.asyncio
async def test_permissions_get_by_id_not_found():
    mock_session = AsyncMock()
    service = PermissionService(mock_session)
...
@pytest.mark.asyncio
async def test_permissions_create_item():
    mock_session = AsyncMock()
    service = PermissionService(mock_session)
    service.repo.create = AsyncMock()

    req = PermissionCreate(name="Enterprise Test", description="Automated Test Entity")
