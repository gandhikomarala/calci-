// DeployModelModal: Promote registered model version to Staging or Production tier

import React, { useState } from 'react';
import { X, Check, AlertCircle } from 'lucide-react';

export interface DeployModelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm?: (data: any) => void;
}

export const DeployModelModal: React.FC<DeployModelModalProps> = ({ isOpen, onClose, onConfirm }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg rounded-xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <h3 className="text-lg font-bold text-slate-100">DeployModel</h3>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-300">Promote registered model version to Staging or Production tier</p>
          </div>
