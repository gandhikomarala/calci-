"""SQLAlchemy 2.0 Mapped Entity Model for RiskScore (Calculated multi-signal transaction risk scores and decisions)."""
...
class RiskScore(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_risk_scores_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<RiskScore(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
