"""CatBoostChurnModel: Categorical target-encoded gradient boosting model interface."""
...
class CatBoostChurnModel(BaseChurnModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "CatBoostChurnModel":
        logger.info("Fitting CatBoostChurnModel", samples=len(X), features=len(X.columns))
        self.model.fit(X, y)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("CatBoostChurnModel must be trained before calling predict_proba()")
