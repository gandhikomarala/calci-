"""Domain Business Logic Service for RiskScore."""
...
logger = get_logger("risk_scores_service")
...
class RiskScoreService:
    ...
    async def list_items(self, params: RiskScoreFilterParams) -> Tuple[Sequence[RiskScore], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing RiskScore items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> RiskScore:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("RiskScore not found", item_id=item_id)
            raise NotFoundError(f"RiskScore with ID '{item_id}' was not found")
        return entity

    async def create(self, req: RiskScoreCreate) -> RiskScore:
        entity = RiskScore()
        ...
        logger.info("RiskScore created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: RiskScoreUpdate) -> RiskScore:
        ...
        logger.info("RiskScore updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("RiskScore deleted successfully", item_id=item_id)
