"""FastAPI REST API Router for ExperimentRun (Individual model training runs, hyperparameters, and durations)."""
...
router = APIRouter(prefix="/experiment_runs", tags=["ExperimentRun"])

@router.get("", response_model=PaginatedResponse[ExperimentRunResponse])
async def list_items(
    ...
    service = ExperimentRunService(session)
    params = ExperimentRunFilterParams(
        page=page,
        page_size=page_size,
        search=search,
        is_active=is_active,
...
@router.get("/{item_id}", response_model=ExperimentRunResponse)
...
async def create_item(req: ExperimentRunCreate, session: AsyncSession = Depends(get_db_session)):
    service = ExperimentRunService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=ExperimentRunResponse)
async def update_item(item_id: str, req: ExperimentRunUpdate, session: AsyncSession = Depends(get_db_session)):
    service = ExperimentRunService(session)
