"""Unit test for Dataset Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.datasets.service import DatasetService
from backend.datasets.schemas import DatasetCreate, DatasetFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_datasets_list_items():
    mock_session = AsyncMock()
    service = DatasetService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = DatasetFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_datasets_get_by_id():
    mock_session = AsyncMock()
    service = DatasetService(mock_session)
...
@pytest.mark.asyncio
async def test_datasets_get_by_id_not_found():
    mock_session = AsyncMock()
    service = DatasetService(mock_session)
...
@pytest.mark.asyncio
async def test_datasets_create_item():
    mock_session = AsyncMock()
    service = DatasetService(mock_session)
    service.repo.create = AsyncMock()

    req = DatasetCreate(name="Enterprise Test", description="Automated Test Entity")
