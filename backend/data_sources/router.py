"""FastAPI REST API Router for DataSource (External data source connectors, database links, and S3 buckets)."""
...
router = APIRouter(prefix="/data_sources", tags=["DataSource"])
...
async def create_item(req: DataSourceCreate, session: AsyncSession = Depends(get_db_session)):
    service = DataSourceService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=DataSourceResponse)
async def update_item(item_id: str, req: DataSourceUpdate, session: AsyncSession = Depends(get_db_session)):
    service = DataSourceService(session)
