...
def test_suite_10_end_to_end_scoring():
    graph = FinancialTransactionGraph()
    biometrics = DeviceBiometricsTransformer(min_samples_for_baseline=2)
    
    # Ingest entities
    graph.add_node("cust_10", "customer", risk_prior=0.05)
    graph.add_node("merchant_10", "merchant", risk_prior=0.01)
    graph.add_edge("cust_10", "merchant_10", "payment", timestamp=1000.0, amount=150.0)
    
    biometrics.update_profile("cust_10", {
    ...
    bio_res = biometrics.calculate_anomaly_score("cust_10", {
    ...
    ego_res = graph.calculate_ego_network_fraud_density("cust_10", depth=2)
