"""SQLAlchemy 2.0 Mapped Entity Model for Prediction (Real-time scored transaction records, probabilities, and latencies)."""
...
class Prediction(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_predictions_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<Prediction(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
