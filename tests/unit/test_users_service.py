"""Unit test for User Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.users.service import UserService
from backend.users.schemas import UserCreate, UserFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_users_list_items():
    mock_session = AsyncMock()
    service = UserService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = UserFilterParams(page=1, page_size=20)
...
@pytest.mark.asyncio
async def test_users_get_by_id_not_found():
    mock_session = AsyncMock()
    service = UserService(mock_session)
...
@pytest.mark.asyncio
async def test_users_create_item():
    mock_session = AsyncMock()
    service = UserService(mock_session)
    service.repo.create = AsyncMock()

    req = UserCreate(name="Enterprise Test", description="Automated Test Entity")
