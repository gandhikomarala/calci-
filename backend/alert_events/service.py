"""Domain Business Logic Service for AlertEvent."""
...
logger = get_logger("alert_events_service")
...
class AlertEventService:
    ...
    async def list_items(self, params: AlertEventFilterParams) -> Tuple[Sequence[AlertEvent], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing AlertEvent items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> AlertEvent:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("AlertEvent not found", item_id=item_id)
            raise NotFoundError(f"AlertEvent with ID '{item_id}' was not found")
        return entity

    async def create(self, req: AlertEventCreate) -> AlertEvent:
        entity = AlertEvent()
        ...
        logger.info("AlertEvent created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: AlertEventUpdate) -> AlertEvent:
        ...
        logger.info("AlertEvent updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("AlertEvent deleted successfully", item_id=item_id)
