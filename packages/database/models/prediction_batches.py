"""SQLAlchemy 2.0 Mapped Entity Model for PredictionBatch (Asynchronous bulk batch prediction jobs)."""
...
class PredictionBatch(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_prediction_batches_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<PredictionBatch(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
