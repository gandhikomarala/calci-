...
def test_suite_01_end_to_end_scoring():
    graph = FinancialTransactionGraph()
    biometrics = DeviceBiometricsTransformer(min_samples_for_baseline=2)
    
    # Ingest entities
    graph.add_node("cust_1", "customer", risk_prior=0.05)
    graph.add_node("merchant_1", "merchant", risk_prior=0.01)
    graph.add_edge("cust_1", "merchant_1", "payment", timestamp=1000.0, amount=150.0)
    
    biometrics.update_profile("cust_1", {
    ...
    bio_res = biometrics.calculate_anomaly_score("cust_1", {
    ...
    ego_res = graph.calculate_ego_network_fraud_density("cust_1", depth=2)
