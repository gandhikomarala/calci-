"""Domain Business Logic Service for InvestigationAssignment."""
...
logger = get_logger("investigation_assignments_service")

class InvestigationAssignmentService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = InvestigationAssignmentRepository(session)

    async def list_items(self, params: InvestigationAssignmentFilterParams) -> Tuple[Sequence[InvestigationAssignment], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing InvestigationAssignment items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> InvestigationAssignment:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("InvestigationAssignment not found", item_id=item_id)
            raise NotFoundError(f"InvestigationAssignment with ID '{item_id}' was not found")
        return entity

    async def create(self, req: InvestigationAssignmentCreate) -> InvestigationAssignment:
        ...
        logger.info("InvestigationAssignment created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: InvestigationAssignmentUpdate) -> InvestigationAssignment:
        ...
        logger.info("InvestigationAssignment updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("InvestigationAssignment deleted successfully", item_id=item_id)
