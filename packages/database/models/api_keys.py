"""SQLAlchemy 2.0 Mapped Entity Model for ApiKey (API keys and institutional access tokens)."""
...
class ApiKey(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_api_keys_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<ApiKey(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
