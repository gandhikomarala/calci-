"""Domain Business Logic Service for User."""
...
class UserService:
    ...
    async def list_items(self, params: UserFilterParams) -> Tuple[Sequence[User], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing User items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> User:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("User not found", item_id=item_id)
            raise NotFoundError(f"User with ID '{item_id}' was not found")
        return entity
    ...
    async def create(self, req: UserCreate) -> User:
        ...
        logger.info("User created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: UserUpdate) -> User:
        ...
        logger.info("User updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("User deleted successfully", item_id=item_id)
