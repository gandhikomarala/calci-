"""High-Performance Asynchronous Repository for Location."""

from typing import Optional, List, Tuple, Sequence, Dict, Any
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select, update, delete, func, desc, asc, and_, or_
from packages.database.models.locations import Location
from packages.database.repository import BaseRepository
from backend.core.logging import get_logger

logger = get_logger("locations_repository")

class LocationRepository(BaseRepository[Location]):
    def __init__(self, session: AsyncSession):
        super().__init__(Location, session)

    async def get_by_code(self, code: str) -> Optional[Location]:
        stmt = select(Location).where(Location.code == code)
        res = await self.session.execute(stmt)
        return res.scalars().first()

    async def list_active(self, skip: int = 0, limit: int = 50) -> Sequence[Location]:
        stmt = select(Location).where(Location.is_active == True).order_by(desc(Location.created_at)).offset(skip).limit(limit)
        res = await self.session.execute(stmt)
        return res.scalars().all()

    async def count_total(self) -> int:
        stmt = select(func.count(Location.id))
        res = await self.session.execute(stmt)
        return res.scalar_one()
