"""Domain Business Logic Service for RiskRuleVersion."""
...
logger = get_logger("risk_rule_versions_service")

class RiskRuleVersionService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = RiskRuleVersionRepository(session)

    async def list_items(self, params: RiskRuleVersionFilterParams) -> Tuple[Sequence[RiskRuleVersion], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing RiskRuleVersion items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> RiskRuleVersion:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("RiskRuleVersion not found", item_id=item_id)
            raise NotFoundError(f"RiskRuleVersion with ID '{item_id}' was not found")
        return entity

    async def create(self, req: RiskRuleVersionCreate) -> RiskRuleVersion:
        ...
        logger.info("RiskRuleVersion created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: RiskRuleVersionUpdate) -> RiskRuleVersion:
        ...
        logger.info("RiskRuleVersion updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("RiskRuleVersion deleted successfully", item_id=item_id)
