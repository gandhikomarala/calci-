"""Domain Business Logic Service for Transaction."""
...
class TransactionService:
    ...
    async def list_items(self, params: TransactionFilterParams) -> Tuple[Sequence[Transaction], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing Transaction items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> Transaction:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("Transaction not found", item_id=item_id)
            raise NotFoundError(f"Transaction with ID '{item_id}' was not found")
        return entity

    async def create(self, req: TransactionCreate) -> Transaction:
        entity = Transaction()
        ...
        logger.info("Transaction created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: TransactionUpdate) -> Transaction:
        ...
        logger.info("Transaction updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("Transaction deleted successfully", item_id=item_id)
