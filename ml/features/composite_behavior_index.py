"""CompositeBehaviorIndexTransformer: Multi-factor weighted composite behavioral deviation score."""
...
logger = get_logger("composite_behavior_index")

class CompositeBehaviorIndexTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, output_col: str = "composite_behavior_index_val", weight: float = 1.0):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> "CompositeBehaviorIndexTransformer":
        logger.info("Fitting CompositeBehaviorIndexTransformer", input_rows=len(X))
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        if not self.is_fitted_:
            raise RuntimeError("CompositeBehaviorIndexTransformer must be fitted before calling transform()")
        
        X_out = X.copy()
        
        # Domain-specific financial fraud feature transformations
        if "composite_behavior_index" == "velocity_5m" and "velocity_last_1h" in X_out.columns:
            v1h = X_out["velocity_last_1h"].fillna(0)
            X_out[self.output_col] = np.clip(v1h * 0.35, 0, 50) * self.weight
        elif "composite_behavior_index" == "amount_deviation" and "amount" in X_out.columns:
        ...
        elif "composite_behavior_index" == "location_anomaly" and "is_location_anomaly" in X_out.columns:
            loc = X_out["is_location_anomaly"].fillna(0)
            X_out[self.output_col] = loc * 2.5 * self.weight
        elif "composite_behavior_index" == "device_risk_scorer" and "is_new_device" in X_out.columns:
        ...
        elif "composite_behavior_index" == "failed_auth_velocity" and "failed_attempts_last_24h" in X_out.columns:
            fails = X_out["failed_attempts_last_24h"].fillna(0)
            X_out[self.output_col] = np.log1p(fails * 2.0) * self.weight
        elif "composite_behavior_index" == "merchant_risk_index" and "merchant_category" in X_out.columns:
