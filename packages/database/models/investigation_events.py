"""SQLAlchemy 2.0 Mapped Entity Model for InvestigationEvent (Chronological audit events inside an investigation case)."""
...
class InvestigationEvent(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_investigation_events_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<InvestigationEvent(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
