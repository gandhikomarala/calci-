"""Domain Authorization & Access Control Policy for InvestigationEvidence."""

from typing import Dict, Any, Optional
from backend.core.enums import UserRole
from backend.core.exceptions import AuthorizationError

class InvestigationEvidencePolicy:
    @staticmethod
    ...
    def assert_can_read(cls, role: UserRole) -> None:
        if not cls.can_read(role):
            raise AuthorizationError("User lacks read permissions for InvestigationEvidence")

    @classmethod
    def assert_can_create(cls, role: UserRole) -> None:
        if not cls.can_create(role):
            raise AuthorizationError("User lacks create permissions for InvestigationEvidence")
