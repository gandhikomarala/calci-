"""RidgeChurnModel: Ridge L2 regularized logistic classifier."""
...
class RidgeChurnModel(BaseChurnModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "RidgeChurnModel":
        logger.info("Fitting RidgeChurnModel", samples=len(X), features=len(X.columns))
        self.model.fit(X, y)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("RidgeChurnModel must be trained before calling predict_proba()")
