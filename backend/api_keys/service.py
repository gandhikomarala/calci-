"""Domain Business Logic Service for ApiKey."""
...
from backend.api_keys.schemas import (
    ApiKeyCreate,
    ApiKeyUpdate,
    ApiKeyResponse,
    ApiKeyFilterParams
...
logger = get_logger("api_keys_service")
...
class ApiKeyService:
    ...
    async def list_items(self, params: ApiKeyFilterParams) -> Tuple[Sequence[ApiKey], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing ApiKey items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> ApiKey:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("ApiKey not found", item_id=item_id)
            raise NotFoundError(f"ApiKey with ID '{item_id}' was not found")
        return entity

    async def create(self, req: ApiKeyCreate) -> ApiKey:
        entity = ApiKey()
        ...
        logger.info("ApiKey created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: ApiKeyUpdate) -> ApiKey:
        ...
        logger.info("ApiKey updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("ApiKey deleted successfully", item_id=item_id)
