"""Unit test for Account Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.accounts.service import AccountService
from backend.accounts.schemas import AccountCreate, AccountFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_accounts_list_items():
    mock_session = AsyncMock()
    service = AccountService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = AccountFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_accounts_get_by_id():
    mock_session = AsyncMock()
    service = AccountService(mock_session)
...
@pytest.mark.asyncio
async def test_accounts_get_by_id_not_found():
    mock_session = AsyncMock()
    service = AccountService(mock_session)
...
@pytest.mark.asyncio
async def test_accounts_create_item():
    mock_session = AsyncMock()
    service = AccountService(mock_session)
    service.repo.create = AsyncMock()

    req = AccountCreate(name="Enterprise Test", description="Automated Test Entity")
