"""Unit test for Transaction Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.transactions.service import TransactionService
from backend.transactions.schemas import TransactionCreate, TransactionFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_transactions_list_items():
    mock_session = AsyncMock()
    service = TransactionService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = TransactionFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_transactions_get_by_id():
    mock_session = AsyncMock()
    service = TransactionService(mock_session)
...
@pytest.mark.asyncio
async def test_transactions_get_by_id_not_found():
    mock_session = AsyncMock()
    service = TransactionService(mock_session)
...
@pytest.mark.asyncio
async def test_transactions_create_item():
    mock_session = AsyncMock()
    service = TransactionService(mock_session)
    service.repo.create = AsyncMock()

    req = TransactionCreate(name="Enterprise Test", description="Automated Test Entity")
