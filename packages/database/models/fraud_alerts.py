"""SQLAlchemy 2.0 Mapped Entity Model for FraudAlert (Real-time fraud alerts triggered by ML scores or rules)."""
...
class FraudAlert(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_fraud_alerts_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<FraudAlert(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
