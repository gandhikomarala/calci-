"""Domain Business Logic Service for Location."""
...
from backend.locations.schemas import (
    LocationCreate,
    LocationUpdate,
    LocationResponse,
...
class LocationService:
    ...
    async def list_items(self, params: LocationFilterParams) -> Tuple[Sequence[Location], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing Location items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> Location:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("Location not found", item_id=item_id)
            raise NotFoundError(f"Location with ID '{item_id}' was not found")
        return entity

    async def create(self, req: LocationCreate) -> Location:
        entity = Location()
        ...
        logger.info("Location created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: LocationUpdate) -> Location:
        ...
        logger.info("Location updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("Location deleted successfully", item_id=item_id)
