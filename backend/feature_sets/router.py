"""FastAPI REST API Router for FeatureSet (Feature store groupings and entity catalog definitions)."""
...
router = APIRouter(prefix="/feature_sets", tags=["FeatureSet"])

@router.get("", response_model=PaginatedResponse[FeatureSetResponse])
async def list_items(
    ...
    service = FeatureSetService(session)
    params = FeatureSetFilterParams(
        page=page,
        page_size=page_size,
        search=search,
        is_active=is_active,
...
@router.get("/{item_id}", response_model=FeatureSetResponse)
...
async def create_item(req: FeatureSetCreate, session: AsyncSession = Depends(get_db_session)):
    service = FeatureSetService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=FeatureSetResponse)
async def update_item(item_id: str, req: FeatureSetUpdate, session: AsyncSession = Depends(get_db_session)):
    service = FeatureSetService(session)
