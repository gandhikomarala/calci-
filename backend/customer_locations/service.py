"""Domain Business Logic Service for CustomerLocation."""
...
logger = get_logger("customer_locations_service")

class CustomerLocationService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = CustomerLocationRepository(session)

    async def list_items(self, params: CustomerLocationFilterParams) -> Tuple[Sequence[CustomerLocation], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing CustomerLocation items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> CustomerLocation:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("CustomerLocation not found", item_id=item_id)
            raise NotFoundError(f"CustomerLocation with ID '{item_id}' was not found")
        return entity

    async def create(self, req: CustomerLocationCreate) -> CustomerLocation:
        ...
        logger.info("CustomerLocation created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: CustomerLocationUpdate) -> CustomerLocation:
        ...
        logger.info("CustomerLocation updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("CustomerLocation deleted successfully", item_id=item_id)
