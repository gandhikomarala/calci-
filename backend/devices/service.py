"""Domain Business Logic Service for Device."""
...
logger = get_logger("devices_service")
...
class DeviceService:
    ...
    async def list_items(self, params: DeviceFilterParams) -> Tuple[Sequence[Device], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing Device items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> Device:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("Device not found", item_id=item_id)
            raise NotFoundError(f"Device with ID '{item_id}' was not found")
        return entity

    async def create(self, req: DeviceCreate) -> Device:
        entity = Device()
        ...
        logger.info("Device created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: DeviceUpdate) -> Device:
        ...
        logger.info("Device updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("Device deleted successfully", item_id=item_id)
