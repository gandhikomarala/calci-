"""Pydantic v2 Schemas for RiskRuleVersion (Immutable version history of risk detection rules)."""

from typing import Optional, List, Dict, Any
import datetime
from pydantic import BaseModel, Field, ConfigDict

class RiskRuleVersionBase(BaseModel):
...
class RiskRuleVersionCreate(RiskRuleVersionBase):
    pass

class RiskRuleVersionUpdate(BaseModel):
    name: Optional[str] = Field(default=None)
    description: Optional[str] = Field(default=None)
    is_active: Optional[bool] = Field(default=None)
    metadata_json: Optional[Dict[str, Any]] = Field(default=None)

class RiskRuleVersionResponse(RiskRuleVersionBase):
