"""Unit test for Investigation Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.investigations.service import InvestigationService
from backend.investigations.schemas import InvestigationCreate, InvestigationFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_investigations_list_items():
...
@pytest.mark.asyncio
async def test_investigations_get_by_id():
    mock_session = AsyncMock()
    service = InvestigationService(mock_session)
...
@pytest.mark.asyncio
async def test_investigations_get_by_id_not_found():
    mock_session = AsyncMock()
    service = InvestigationService(mock_session)
...
@pytest.mark.asyncio
async def test_investigations_create_item():
    mock_session = AsyncMock()
    service = InvestigationService(mock_session)
    service.repo.create = AsyncMock()

    req = InvestigationCreate(name="Enterprise Test", description="Automated Test Entity")
