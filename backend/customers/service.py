"""Domain Business Logic Service for Customer."""
...
from backend.customers.schemas import (
    CustomerCreate,
    CustomerUpdate,
    CustomerResponse,
...
class CustomerService:
    ...
    async def list_items(self, params: CustomerFilterParams) -> Tuple[Sequence[Customer], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing Customer items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> Customer:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("Customer not found", item_id=item_id)
            raise NotFoundError(f"Customer with ID '{item_id}' was not found")
        return entity

    async def create(self, req: CustomerCreate) -> Customer:
        entity = Customer()
        ...
        logger.info("Customer created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: CustomerUpdate) -> Customer:
        ...
        logger.info("Customer updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("Customer deleted successfully", item_id=item_id)
