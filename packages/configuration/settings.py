"""Strongly typed application configuration settings using Pydantic v2."""

from functools import lru_cache
from typing import List, Optional
from pydantic import Field
from pydantic_settings import BaseSettings, SettingsConfigDict
from packages.core.enums import EnvironmentType
...
class Settings(BaseSettings):
    ...
    APP_NAME: str = Field(default="Customer Churn Prediction & MLOps Platform")
    APP_HOST: str = Field(default="0.0.0.0")
    APP_PORT: int = Field(default=8000)
    API_V1_PREFIX: str = Field(default="/api/v1")
    SECRET_KEY: str = Field(default="enterprise-mlops-default-secret-key-32-chars-long")
    ...
    POSTGRES_USER: str = Field(default="churn_admin")
    POSTGRES_PASSWORD: str = Field(default="churn_secure_password_2026")
    POSTGRES_DB: str = Field(default="churn_platform_db")
    ...
    S3_BUCKET_NAME: Optional[str] = Field(default="enterprise-churn-mlops-artifacts")

    # ML & Risk Thresholds
    CHURN_RISK_LOW_THRESHOLD: float = Field(default=0.30)
    CHURN_RISK_HIGH_THRESHOLD: float = Field(default=0.70)
