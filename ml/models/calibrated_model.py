"""ProbabilityCalibratedChurnModel: Platt scaling and isotonic probability calibration wrapper."""
...
class ProbabilityCalibratedChurnModel(BaseChurnModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "ProbabilityCalibratedChurnModel":
        logger.info("Fitting ProbabilityCalibratedChurnModel", samples=len(X), features=len(X.columns))
        self.model.fit(X, y)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("ProbabilityCalibratedChurnModel must be trained before calling predict_proba()")
