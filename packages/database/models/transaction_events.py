"""SQLAlchemy 2.0 Mapped Entity Model for TransactionEvent (Transaction state transitions, webhooks, and ledger events)."""
...
class TransactionEvent(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_transaction_events_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<TransactionEvent(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
