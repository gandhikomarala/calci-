"""Domain Business Logic Service for ExperimentParameter."""
...
logger = get_logger("experiment_parameters_service")

class ExperimentParameterService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = ExperimentParameterRepository(session)

    async def list_items(self, params: ExperimentParameterFilterParams) -> Tuple[Sequence[ExperimentParameter], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing ExperimentParameter items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> ExperimentParameter:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("ExperimentParameter not found", item_id=item_id)
            raise NotFoundError(f"ExperimentParameter with ID '{item_id}' was not found")
        return entity

    async def create(self, req: ExperimentParameterCreate) -> ExperimentParameter:
        ...
        logger.info("ExperimentParameter created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: ExperimentParameterUpdate) -> ExperimentParameter:
        ...
        logger.info("ExperimentParameter updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("ExperimentParameter deleted successfully", item_id=item_id)
