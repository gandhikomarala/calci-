"""SQLAlchemy 2.0 Mapped Entity Model for RefreshToken (JWT refresh tokens and revocation registry)."""
...
class RefreshToken(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_refresh_tokens_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<RefreshToken(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
