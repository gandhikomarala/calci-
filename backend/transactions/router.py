"""FastAPI REST API Router for Transaction (Financial transactions, amounts, channels, payment methods, timestamps)."""
...
async def create_item(req: TransactionCreate, session: AsyncSession = Depends(get_db_session)):
    service = TransactionService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=TransactionResponse)
async def update_item(item_id: str, req: TransactionUpdate, session: AsyncSession = Depends(get_db_session)):
    service = TransactionService(session)
