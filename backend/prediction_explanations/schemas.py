"""Pydantic v2 Schemas for PredictionExplanation (TreeSHAP and KernelSHAP feature contribution values)."""

from typing import Optional, List, Dict, Any
import datetime
from pydantic import BaseModel, Field, ConfigDict

class PredictionExplanationBase(BaseModel):
...
class PredictionExplanationCreate(PredictionExplanationBase):
    pass

class PredictionExplanationUpdate(BaseModel):
    name: Optional[str] = Field(default=None)
    description: Optional[str] = Field(default=None)
    is_active: Optional[bool] = Field(default=None)
    metadata_json: Optional[Dict[str, Any]] = Field(default=None)

class PredictionExplanationResponse(PredictionExplanationBase):
...
class PredictionExplanationDetailResponse(PredictionExplanationResponse):
    extra_attributes: Optional[Dict[str, Any]] = Field(default=None)
    audit_history: Optional[List[Dict[str, Any]]] = Field(default=None)

class PredictionExplanationFilterParams(BaseModel):
