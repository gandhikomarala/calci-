"""SQLAlchemy 2.0 Mapped Entity Model for AuditLog (Immutable security and compliance audit trail events)."""
...
class AuditLog(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_audit_logs_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<AuditLog(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
