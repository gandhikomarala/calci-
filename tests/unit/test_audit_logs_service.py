"""Unit test for AuditLog Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.audit_logs.service import AuditLogService
from backend.audit_logs.schemas import AuditLogCreate, AuditLogFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_audit_logs_list_items():
    mock_session = AsyncMock()
    service = AuditLogService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = AuditLogFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_audit_logs_get_by_id():
    mock_session = AsyncMock()
    service = AuditLogService(mock_session)
...
@pytest.mark.asyncio
async def test_audit_logs_get_by_id_not_found():
    mock_session = AsyncMock()
    service = AuditLogService(mock_session)
...
@pytest.mark.asyncio
async def test_audit_logs_create_item():
    mock_session = AsyncMock()
    service = AuditLogService(mock_session)
    service.repo.create = AsyncMock()

    req = AuditLogCreate(name="Enterprise Test", description="Automated Test Entity")
