"""SQLAlchemy 2.0 Mapped Entity Model for Device (Hardware fingerprints, emulator detection, and IP associations)."""
...
class Device(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_devices_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<Device(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
