"""CostSensitiveOptimalThresholdModel: Cost-matrix optimized threshold churn classifier."""
...
class CostSensitiveOptimalThresholdModel(BaseChurnModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "CostSensitiveOptimalThresholdModel":
        logger.info("Fitting CostSensitiveOptimalThresholdModel", samples=len(X), features=len(X.columns))
        self.model.fit(X, y)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("CostSensitiveOptimalThresholdModel must be trained before calling predict_proba()")
