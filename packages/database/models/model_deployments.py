"""SQLAlchemy 2.0 Mapped Entity Model for ModelDeployment (Active deployment environments (Staging, Production, Canary))."""
...
class ModelDeployment(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_model_deployments_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<ModelDeployment(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
