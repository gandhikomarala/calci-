"""Domain Business Logic Service for MLModel."""
...
logger = get_logger("models_service")
...
class MLModelService:
    ...
    async def list_items(self, params: MLModelFilterParams) -> Tuple[Sequence[MLModel], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing MLModel items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> MLModel:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("MLModel not found", item_id=item_id)
            raise NotFoundError(f"MLModel with ID '{item_id}' was not found")
        return entity

    async def create(self, req: MLModelCreate) -> MLModel:
        entity = MLModel()
        ...
        logger.info("MLModel created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: MLModelUpdate) -> MLModel:
        ...
        logger.info("MLModel updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("MLModel deleted successfully", item_id=item_id)
