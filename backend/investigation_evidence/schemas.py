"""Pydantic v2 Schemas for InvestigationEvidence (Evidence items (transaction screenshots, IP logs, device traces))."""
...
class InvestigationEvidenceDetailResponse(InvestigationEvidenceResponse):
    extra_attributes: Optional[Dict[str, Any]] = Field(default=None)
    audit_history: Optional[List[Dict[str, Any]]] = Field(default=None)

class InvestigationEvidenceFilterParams(BaseModel):
