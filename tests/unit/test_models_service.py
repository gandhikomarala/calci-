"""Unit test for MLModel Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.models.service import MLModelService
from backend.models.schemas import MLModelCreate, MLModelFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_models_list_items():
    mock_session = AsyncMock()
    service = MLModelService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = MLModelFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_models_get_by_id():
    mock_session = AsyncMock()
    service = MLModelService(mock_session)
...
@pytest.mark.asyncio
async def test_models_get_by_id_not_found():
    mock_session = AsyncMock()
    service = MLModelService(mock_session)
...
@pytest.mark.asyncio
async def test_models_create_item():
    mock_session = AsyncMock()
    service = MLModelService(mock_session)
    service.repo.create = AsyncMock()

    req = MLModelCreate(name="Enterprise Test", description="Automated Test Entity")
