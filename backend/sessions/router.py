"""FastAPI REST API Router for UserSession (Active web session tracking and concurrent login controls)."""
...
router = APIRouter(prefix="/sessions", tags=["UserSession"])

@router.get("", response_model=PaginatedResponse[UserSessionResponse])
async def list_items(
    ...
    params = UserSessionFilterParams(
        page=page,
        page_size=page_size,
        search=search,
        is_active=is_active,
...
@router.get("/{item_id}", response_model=UserSessionResponse)
...
async def create_item(req: UserSessionCreate, session: AsyncSession = Depends(get_db_session)):
    service = UserSessionService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=UserSessionResponse)
async def update_item(item_id: str, req: UserSessionUpdate, session: AsyncSession = Depends(get_db_session)):
    service = UserSessionService(session)
