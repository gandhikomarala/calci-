"""SQLAlchemy 2.0 Mapped Entity Model for DatasetVersion (Immutable dataset version snapshots with row counts and hashes)."""
...
class DatasetVersion(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_dataset_versions_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<DatasetVersion(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
