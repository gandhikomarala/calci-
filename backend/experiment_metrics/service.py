"""Domain Business Logic Service for ExperimentMetric."""
...
logger = get_logger("experiment_metrics_service")

class ExperimentMetricService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = ExperimentMetricRepository(session)

    async def list_items(self, params: ExperimentMetricFilterParams) -> Tuple[Sequence[ExperimentMetric], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing ExperimentMetric items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> ExperimentMetric:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("ExperimentMetric not found", item_id=item_id)
            raise NotFoundError(f"ExperimentMetric with ID '{item_id}' was not found")
        return entity

    async def create(self, req: ExperimentMetricCreate) -> ExperimentMetric:
        ...
        logger.info("ExperimentMetric created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: ExperimentMetricUpdate) -> ExperimentMetric:
        ...
        logger.info("ExperimentMetric updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("ExperimentMetric deleted successfully", item_id=item_id)
