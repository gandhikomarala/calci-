"""Domain Business Logic Service for CustomerDevice."""
...
logger = get_logger("customer_devices_service")

class CustomerDeviceService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = CustomerDeviceRepository(session)

    async def list_items(self, params: CustomerDeviceFilterParams) -> Tuple[Sequence[CustomerDevice], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing CustomerDevice items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> CustomerDevice:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("CustomerDevice not found", item_id=item_id)
            raise NotFoundError(f"CustomerDevice with ID '{item_id}' was not found")
        return entity

    async def create(self, req: CustomerDeviceCreate) -> CustomerDevice:
        ...
        logger.info("CustomerDevice created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: CustomerDeviceUpdate) -> CustomerDevice:
        ...
        logger.info("CustomerDevice updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("CustomerDevice deleted successfully", item_id=item_id)
