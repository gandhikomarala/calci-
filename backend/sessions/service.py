"""Domain Business Logic Service for UserSession."""
...
class UserSessionService:
    ...
    async def list_items(self, params: UserSessionFilterParams) -> Tuple[Sequence[UserSession], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing UserSession items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> UserSession:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("UserSession not found", item_id=item_id)
            raise NotFoundError(f"UserSession with ID '{item_id}' was not found")
        return entity

    async def create(self, req: UserSessionCreate) -> UserSession:
        entity = UserSession()
        ...
        logger.info("UserSession created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: UserSessionUpdate) -> UserSession:
        ...
        logger.info("UserSession updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("UserSession deleted successfully", item_id=item_id)
