"""Domain Business Logic Service for Permission."""
...
logger = get_logger("permissions_service")
...
class PermissionService:
    ...
    async def list_items(self, params: PermissionFilterParams) -> Tuple[Sequence[Permission], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing Permission items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> Permission:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("Permission not found", item_id=item_id)
            raise NotFoundError(f"Permission with ID '{item_id}' was not found")
        return entity

    async def create(self, req: PermissionCreate) -> Permission:
        entity = Permission()
        ...
        logger.info("Permission created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: PermissionUpdate) -> Permission:
        ...
        logger.info("Permission updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("Permission deleted successfully", item_id=item_id)
