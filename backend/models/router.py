"""FastAPI REST API Router for MLModel (Registered machine learning model families)."""
...
router = APIRouter(prefix="/models", tags=["MLModel"])
...
async def list_items(
    ...
    service = MLModelService(session)
    params = MLModelFilterParams(
        page=page,
        page_size=page_size,
        search=search,
        is_active=is_active,
...
@router.get("/{item_id}", response_model=MLModelResponse)
...
async def create_item(req: MLModelCreate, session: AsyncSession = Depends(get_db_session)):
    service = MLModelService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=MLModelResponse)
async def update_item(item_id: str, req: MLModelUpdate, session: AsyncSession = Depends(get_db_session)):
    service = MLModelService(session)
