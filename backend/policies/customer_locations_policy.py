"""Domain Authorization & Access Control Policy for CustomerLocation."""
...
class CustomerLocationPolicy:
    ...
    def assert_can_read(cls, role: UserRole) -> None:
        if not cls.can_read(role):
            raise AuthorizationError("User lacks read permissions for CustomerLocation")

    @classmethod
    def assert_can_create(cls, role: UserRole) -> None:
        if not cls.can_create(role):
            raise AuthorizationError("User lacks create permissions for CustomerLocation")
