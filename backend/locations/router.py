"""FastAPI REST API Router for Location (Geographic locations, coordinates, IP geolocation, geohashes)."""
...
async def create_item(req: LocationCreate, session: AsyncSession = Depends(get_db_session)):
    service = LocationService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=LocationResponse)
async def update_item(item_id: str, req: LocationUpdate, session: AsyncSession = Depends(get_db_session)):
    service = LocationService(session)
