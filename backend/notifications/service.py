"""Domain Business Logic Service for Notification."""
...
class NotificationService:
    ...
    async def list_items(self, params: NotificationFilterParams) -> Tuple[Sequence[Notification], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing Notification items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> Notification:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("Notification not found", item_id=item_id)
            raise NotFoundError(f"Notification with ID '{item_id}' was not found")
        return entity

    async def create(self, req: NotificationCreate) -> Notification:
        entity = Notification()
        ...
        logger.info("Notification created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: NotificationUpdate) -> Notification:
        ...
        logger.info("Notification updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("Notification deleted successfully", item_id=item_id)
