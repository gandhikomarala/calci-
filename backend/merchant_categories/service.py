"""Domain Business Logic Service for MerchantCategory."""
...
logger = get_logger("merchant_categories_service")

class MerchantCategoryService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = MerchantCategoryRepository(session)

    async def list_items(self, params: MerchantCategoryFilterParams) -> Tuple[Sequence[MerchantCategory], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing MerchantCategory items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> MerchantCategory:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("MerchantCategory not found", item_id=item_id)
            raise NotFoundError(f"MerchantCategory with ID '{item_id}' was not found")
        return entity

    async def create(self, req: MerchantCategoryCreate) -> MerchantCategory:
        ...
        logger.info("MerchantCategory created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: MerchantCategoryUpdate) -> MerchantCategory:
        ...
        logger.info("MerchantCategory updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("MerchantCategory deleted successfully", item_id=item_id)
