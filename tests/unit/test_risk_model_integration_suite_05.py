...
def test_suite_05_end_to_end_scoring():
    graph = FinancialTransactionGraph()
    biometrics = DeviceBiometricsTransformer(min_samples_for_baseline=2)
    
    # Ingest entities
    graph.add_node("cust_5", "customer", risk_prior=0.05)
    graph.add_node("merchant_5", "merchant", risk_prior=0.01)
    graph.add_edge("cust_5", "merchant_5", "payment", timestamp=1000.0, amount=150.0)
    
    biometrics.update_profile("cust_5", {
    ...
    bio_res = biometrics.calculate_anomaly_score("cust_5", {
    ...
    ego_res = graph.calculate_ego_network_fraud_density("cust_5", depth=2)
