"""SQLAlchemy 2.0 Mapped Entity Model for Transaction (Financial transactions, amounts, channels, and payment instruments)."""
...
class Transaction(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_transactions_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<Transaction(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
