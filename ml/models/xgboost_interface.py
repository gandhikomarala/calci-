"""XGBoostChurnModel: Extreme Gradient Boosted trees classifier interface."""
...
class XGBoostChurnModel(BaseChurnModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "XGBoostChurnModel":
        logger.info("Fitting XGBoostChurnModel", samples=len(X), features=len(X.columns))
        self.model.fit(X, y)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("XGBoostChurnModel must be trained before calling predict_proba()")
