"""VotingEnsembleChurnModel: Soft probability averaging voting ensemble."""
...
class VotingEnsembleChurnModel(BaseChurnModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "VotingEnsembleChurnModel":
        logger.info("Fitting VotingEnsembleChurnModel", samples=len(X), features=len(X.columns))
        self.model.fit(X, y)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("VotingEnsembleChurnModel must be trained before calling predict_proba()")
