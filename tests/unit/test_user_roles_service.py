"""Unit test for UserRole Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.user_roles.service import UserRoleService
from backend.user_roles.schemas import UserRoleCreate, UserRoleFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_user_roles_list_items():
    mock_session = AsyncMock()
    service = UserRoleService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = UserRoleFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_user_roles_get_by_id():
    mock_session = AsyncMock()
    service = UserRoleService(mock_session)
...
@pytest.mark.asyncio
async def test_user_roles_get_by_id_not_found():
    mock_session = AsyncMock()
    service = UserRoleService(mock_session)
...
@pytest.mark.asyncio
async def test_user_roles_create_item():
    mock_session = AsyncMock()
    service = UserRoleService(mock_session)
    service.repo.create = AsyncMock()

    req = UserRoleCreate(name="Enterprise Test", description="Automated Test Entity")
