"""FastAPI REST API Router for Customer (Banking customer entities, KYC status, and risk categorizations)."""
...
async def create_item(req: CustomerCreate, session: AsyncSession = Depends(get_db_session)):
    service = CustomerService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=CustomerResponse)
async def update_item(item_id: str, req: CustomerUpdate, session: AsyncSession = Depends(get_db_session)):
    service = CustomerService(session)
