"""SQLAlchemy 2.0 Mapped Entity Model for Dataset (Financial dataset metadata, file paths, and schemas)."""
...
class Dataset(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_datasets_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<Dataset(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
