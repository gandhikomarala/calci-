"""FrequencyScorer: Frequency calculation based on login and session counts over 30d/60d/90d windows."""
...
class FrequencyScorer(BaseEstimator, TransformerMixin):
    def __init__(self, output_col: str = "frequency_scorer_val", weight: float = 1.0):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> "FrequencyScorer":
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        if not self.is_fitted_:
            raise RuntimeError("FrequencyScorer must be fitted before transform")
        X_out = X.copy()
        
        # Realistic business domain transformation logic
        if "frequency_scorer" == "recency_scorer" and "days_since_last_activity" in X_out.columns:
            days = X_out["days_since_last_activity"].fillna(30)
            X_out[self.output_col] = np.exp(-days / 30.0) * self.weight
        elif "frequency_scorer" == "frequency_scorer" and "login_count_last_30d" in X_out.columns:
            logins = X_out["login_count_last_30d"].fillna(0)
            X_out[self.output_col] = np.log1p(logins) * self.weight
        elif "frequency_scorer" == "monetary_scorer" and "total_spend" in X_out.columns:
            spend = X_out["total_spend"].fillna(0)
            X_out[self.output_col] = np.log1p(spend / 100.0) * self.weight
        elif "frequency_scorer" == "usage_velocity" and "usage_trend" in X_out.columns:
            trend = X_out["usage_trend"].fillna(0)
            X_out[self.output_col] = np.clip(trend * 2.0, -1.0, 1.0) * self.weight
        elif "frequency_scorer" == "support_dissatisfaction" and "satisfaction_score" in X_out.columns:
        ...
        elif "frequency_scorer" == "payment_reliability" and "payment_failures_last_12m" in X_out.columns:
            failures = X_out["payment_failures_last_12m"].fillna(0)
            X_out[self.output_col] = np.clip(1.0 - (failures * 0.25), 0.0, 1.0) * self.weight
        elif "frequency_scorer" == "engagement_decay" and "email_open_rate" in X_out.columns:
