"""Domain Business Logic Service for RefreshToken."""
...
logger = get_logger("refresh_tokens_service")
...
class RefreshTokenService:
    ...
    async def list_items(self, params: RefreshTokenFilterParams) -> Tuple[Sequence[RefreshToken], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing RefreshToken items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> RefreshToken:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("RefreshToken not found", item_id=item_id)
            raise NotFoundError(f"RefreshToken with ID '{item_id}' was not found")
        return entity

    async def create(self, req: RefreshTokenCreate) -> RefreshToken:
        entity = RefreshToken()
        ...
        logger.info("RefreshToken created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: RefreshTokenUpdate) -> RefreshToken:
        ...
        logger.info("RefreshToken updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("RefreshToken deleted successfully", item_id=item_id)
