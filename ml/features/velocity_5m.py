"""Velocity5mTransformer: Transaction count and cumulative volume over rolling 5-minute window."""
...
logger = get_logger("velocity_5m")

class Velocity5mTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, output_col: str = "velocity_5m_val", weight: float = 1.0):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> "Velocity5mTransformer":
        logger.info("Fitting Velocity5mTransformer", input_rows=len(X))
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        if not self.is_fitted_:
            raise RuntimeError("Velocity5mTransformer must be fitted before calling transform()")
        
        X_out = X.copy()
        
        # Domain-specific financial fraud feature transformations
        if "velocity_5m" == "velocity_5m" and "velocity_last_1h" in X_out.columns:
            v1h = X_out["velocity_last_1h"].fillna(0)
            X_out[self.output_col] = np.clip(v1h * 0.35, 0, 50) * self.weight
        elif "velocity_5m" == "amount_deviation" and "amount" in X_out.columns:
        ...
        elif "velocity_5m" == "location_anomaly" and "is_location_anomaly" in X_out.columns:
            loc = X_out["is_location_anomaly"].fillna(0)
            X_out[self.output_col] = loc * 2.5 * self.weight
        elif "velocity_5m" == "device_risk_scorer" and "is_new_device" in X_out.columns:
        ...
        elif "velocity_5m" == "failed_auth_velocity" and "failed_attempts_last_24h" in X_out.columns:
            fails = X_out["failed_attempts_last_24h"].fillna(0)
            X_out[self.output_col] = np.log1p(fails * 2.0) * self.weight
        elif "velocity_5m" == "merchant_risk_index" and "merchant_category" in X_out.columns:
