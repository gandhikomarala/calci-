"""FastAPI REST API Router for ApiKey (Developer and institutional API credentials)."""
...
router = APIRouter(prefix="/api_keys", tags=["ApiKey"])
...
@router.get("/{item_id}", response_model=ApiKeyResponse)
async def get_by_id(item_id: str, session: AsyncSession = Depends(get_db_session)):
    service = ApiKeyService(session)
    return await service.get_by_id(item_id)

@router.post("", response_model=ApiKeyResponse, status_code=status.HTTP_201_CREATED)
async def create_item(req: ApiKeyCreate, session: AsyncSession = Depends(get_db_session)):
    service = ApiKeyService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=ApiKeyResponse)
async def update_item(item_id: str, req: ApiKeyUpdate, session: AsyncSession = Depends(get_db_session)):
    service = ApiKeyService(session)
