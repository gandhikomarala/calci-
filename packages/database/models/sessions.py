"""SQLAlchemy 2.0 Mapped Entity Model for UserSession (Active analyst login sessions and device telemetry)."""
...
class UserSession(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_user_sessions_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<UserSession(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
