"""SQLAlchemy 2.0 Mapped Entity Model for Location (Geographic locations, coordinates, and IP geolocations)."""
...
class Location(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_locations_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<Location(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
