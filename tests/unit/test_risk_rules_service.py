"""Unit test for RiskRule Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.risk_rules.service import RiskRuleService
from backend.risk_rules.schemas import RiskRuleCreate, RiskRuleFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_risk_rules_list_items():
    mock_session = AsyncMock()
    service = RiskRuleService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = RiskRuleFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_risk_rules_get_by_id():
    mock_session = AsyncMock()
    service = RiskRuleService(mock_session)
...
@pytest.mark.asyncio
async def test_risk_rules_get_by_id_not_found():
    mock_session = AsyncMock()
    service = RiskRuleService(mock_session)
...
@pytest.mark.asyncio
async def test_risk_rules_create_item():
    mock_session = AsyncMock()
    service = RiskRuleService(mock_session)
    service.repo.create = AsyncMock()

    req = RiskRuleCreate(name="Enterprise Test", description="Automated Test Entity")
