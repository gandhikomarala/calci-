"""Unit test for Customer Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.customers.service import CustomerService
from backend.customers.schemas import CustomerCreate, CustomerFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_customers_list_items():
    mock_session = AsyncMock()
    service = CustomerService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = CustomerFilterParams(page=1, page_size=20)
...
@pytest.mark.asyncio
async def test_customers_get_by_id_not_found():
    mock_session = AsyncMock()
    service = CustomerService(mock_session)
...
@pytest.mark.asyncio
async def test_customers_create_item():
    mock_session = AsyncMock()
    service = CustomerService(mock_session)
    service.repo.create = AsyncMock()

    req = CustomerCreate(name="Enterprise Test", description="Automated Test Entity")
