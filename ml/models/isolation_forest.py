"""IsolationForestAnomalyModel: Unsupervised Isolation Forest Transaction Anomaly Detector."""
...
class IsolationForestAnomalyModel:
    def __init__(self, hyperparameters: Optional[Dict[str, Any]] = None):
        ...
        if "isolation_forest" == "lightgbm_model":
        ...
        elif "isolation_forest" == "random_forest":
        ...
        elif "isolation_forest" == "gradient_boosting":
            self.model = GradientBoostingClassifier(
                n_estimators=self.hyperparameters.get("n_estimators", 100),
                learning_rate=self.hyperparameters.get("learning_rate", 0.08),
                random_state=42
            )
        elif "isolation_forest" == "isolation_forest":
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> "IsolationForestAnomalyModel":
        logger.info("Training IsolationForestAnomalyModel", samples=len(X), features=len(X.columns))
        if "isolation_forest" == "isolation_forest":
            self.model.fit(X)
        else:
            if y is None:
        ...
        logger.info("IsolationForestAnomalyModel training complete")
        return self

    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("IsolationForestAnomalyModel must be trained before calling predict_proba()")
