"""Domain Business Logic Service for PredictionExplanation."""
...
logger = get_logger("prediction_explanations_service")

class PredictionExplanationService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = PredictionExplanationRepository(session)

    async def list_items(self, params: PredictionExplanationFilterParams) -> Tuple[Sequence[PredictionExplanation], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing PredictionExplanation items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> PredictionExplanation:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("PredictionExplanation not found", item_id=item_id)
            raise NotFoundError(f"PredictionExplanation with ID '{item_id}' was not found")
        return entity

    async def create(self, req: PredictionExplanationCreate) -> PredictionExplanation:
        ...
        logger.info("PredictionExplanation created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: PredictionExplanationUpdate) -> PredictionExplanation:
        ...
        logger.info("PredictionExplanation updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("PredictionExplanation deleted successfully", item_id=item_id)
