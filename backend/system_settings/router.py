"""FastAPI REST API Router for SystemSetting (Global runtime configuration, risk thresholds, and rate limits)."""
...
router = APIRouter(prefix="/system_settings", tags=["SystemSetting"])
...
async def list_items(
    ...
    params = SystemSettingFilterParams(
        page=page,
        page_size=page_size,
        search=search,
        is_active=is_active,
...
@router.get("/{item_id}", response_model=SystemSettingResponse)
...
async def create_item(req: SystemSettingCreate, session: AsyncSession = Depends(get_db_session)):
    service = SystemSettingService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=SystemSettingResponse)
async def update_item(item_id: str, req: SystemSettingUpdate, session: AsyncSession = Depends(get_db_session)):
    service = SystemSettingService(session)
