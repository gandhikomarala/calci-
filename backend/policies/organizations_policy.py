"""Domain Authorization & Access Control Policy for Organization."""
...
class OrganizationPolicy:
    ...
    def assert_can_read(cls, role: UserRole) -> None:
        if not cls.can_read(role):
            raise AuthorizationError("User lacks read permissions for Organization")

    @classmethod
    def assert_can_create(cls, role: UserRole) -> None:
        if not cls.can_create(role):
            raise AuthorizationError("User lacks create permissions for Organization")
