"""FastAPI REST API Router for Dataset (Financial dataset metadata, source declarations, and schemas)."""
...
@router.get("/{item_id}", response_model=DatasetResponse)
async def get_by_id(item_id: str, session: AsyncSession = Depends(get_db_session)):
    service = DatasetService(session)
    return await service.get_by_id(item_id)

@router.post("", response_model=DatasetResponse, status_code=status.HTTP_201_CREATED)
async def create_item(req: DatasetCreate, session: AsyncSession = Depends(get_db_session)):
    service = DatasetService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=DatasetResponse)
async def update_item(item_id: str, req: DatasetUpdate, session: AsyncSession = Depends(get_db_session)):
    service = DatasetService(session)
