"""Domain Business Logic Service for Dataset."""
...
logger = get_logger("datasets_service")
...
class DatasetService:
    ...
    async def list_items(self, params: DatasetFilterParams) -> Tuple[Sequence[Dataset], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing Dataset items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> Dataset:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("Dataset not found", item_id=item_id)
            raise NotFoundError(f"Dataset with ID '{item_id}' was not found")
        return entity

    async def create(self, req: DatasetCreate) -> Dataset:
        entity = Dataset()
        ...
        logger.info("Dataset created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: DatasetUpdate) -> Dataset:
        ...
        logger.info("Dataset updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("Dataset deleted successfully", item_id=item_id)
