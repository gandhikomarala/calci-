"""Pydantic v2 Schemas for InvestigationComment (Internal analyst collaboration notes and case findings)."""

from typing import Optional, List, Dict, Any
import datetime
from pydantic import BaseModel, Field, ConfigDict

class InvestigationCommentBase(BaseModel):
...
class InvestigationCommentCreate(InvestigationCommentBase):
    pass

class InvestigationCommentUpdate(BaseModel):
    name: Optional[str] = Field(default=None)
    description: Optional[str] = Field(default=None)
    is_active: Optional[bool] = Field(default=None)
    metadata_json: Optional[Dict[str, Any]] = Field(default=None)

class InvestigationCommentResponse(InvestigationCommentBase):
...
class InvestigationCommentDetailResponse(InvestigationCommentResponse):
    extra_attributes: Optional[Dict[str, Any]] = Field(default=None)
    audit_history: Optional[List[Dict[str, Any]]] = Field(default=None)

class InvestigationCommentFilterParams(BaseModel):
