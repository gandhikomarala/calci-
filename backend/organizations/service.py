"""Domain Business Logic Service for Organization."""
...
logger = get_logger("organizations_service")
...
class OrganizationService:
    ...
    async def list_items(self, params: OrganizationFilterParams) -> Tuple[Sequence[Organization], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing Organization items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> Organization:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("Organization not found", item_id=item_id)
            raise NotFoundError(f"Organization with ID '{item_id}' was not found")
        return entity

    async def create(self, req: OrganizationCreate) -> Organization:
        entity = Organization()
        ...
        logger.info("Organization created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: OrganizationUpdate) -> Organization:
        ...
        logger.info("Organization updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("Organization deleted successfully", item_id=item_id)
