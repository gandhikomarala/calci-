"""Unit test for Location Domain Service."""

import pytest
from unittest.mock import AsyncMock, MagicMock
from backend.locations.service import LocationService
from backend.locations.schemas import LocationCreate, LocationFilterParams
from backend.core.exceptions import NotFoundError

@pytest.mark.asyncio
async def test_locations_list_items():
    mock_session = AsyncMock()
    service = LocationService(mock_session)
    service.repo.list_filtered = AsyncMock(return_value=([], 0))

    params = LocationFilterParams(page=1, page_size=20)
    items, total = await service.list_items(params)
    assert items == []
    assert total == 0

@pytest.mark.asyncio
async def test_locations_get_by_id():
    mock_session = AsyncMock()
    service = LocationService(mock_session)
...
@pytest.mark.asyncio
async def test_locations_get_by_id_not_found():
    mock_session = AsyncMock()
    service = LocationService(mock_session)
...
@pytest.mark.asyncio
async def test_locations_create_item():
    mock_session = AsyncMock()
    service = LocationService(mock_session)
    service.repo.create = AsyncMock()

    req = LocationCreate(name="Enterprise Test", description="Automated Test Entity")
