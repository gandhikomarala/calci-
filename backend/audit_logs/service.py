"""Domain Business Logic Service for AuditLog."""
...
logger = get_logger("audit_logs_service")
...
class AuditLogService:
    ...
    async def list_items(self, params: AuditLogFilterParams) -> Tuple[Sequence[AuditLog], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing AuditLog items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> AuditLog:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("AuditLog not found", item_id=item_id)
            raise NotFoundError(f"AuditLog with ID '{item_id}' was not found")
        return entity

    async def create(self, req: AuditLogCreate) -> AuditLog:
        entity = AuditLog()
        ...
        logger.info("AuditLog created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: AuditLogUpdate) -> AuditLog:
        ...
        logger.info("AuditLog updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("AuditLog deleted successfully", item_id=item_id)
