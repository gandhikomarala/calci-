"""FastAPI REST API Router for Investigation (Analyst fraud case management, investigation workflows, and dossiers)."""
...
@router.get("/{item_id}", response_model=InvestigationResponse)
async def get_by_id(item_id: str, session: AsyncSession = Depends(get_db_session)):
    service = InvestigationService(session)
    return await service.get_by_id(item_id)

@router.post("", response_model=InvestigationResponse, status_code=status.HTTP_201_CREATED)
async def create_item(req: InvestigationCreate, session: AsyncSession = Depends(get_db_session)):
    service = InvestigationService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=InvestigationResponse)
async def update_item(item_id: str, req: InvestigationUpdate, session: AsyncSession = Depends(get_db_session)):
    service = InvestigationService(session)
