"""Domain Business Logic Service for ModelArtifact."""
...
logger = get_logger("model_artifacts_service")

class ModelArtifactService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = ModelArtifactRepository(session)

    async def list_items(self, params: ModelArtifactFilterParams) -> Tuple[Sequence[ModelArtifact], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing ModelArtifact items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> ModelArtifact:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("ModelArtifact not found", item_id=item_id)
            raise NotFoundError(f"ModelArtifact with ID '{item_id}' was not found")
        return entity

    async def create(self, req: ModelArtifactCreate) -> ModelArtifact:
        ...
        logger.info("ModelArtifact created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: ModelArtifactUpdate) -> ModelArtifact:
        ...
        logger.info("ModelArtifact updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("ModelArtifact deleted successfully", item_id=item_id)
