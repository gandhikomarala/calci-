"""Domain Business Logic Service for InvestigationEvidence."""
...
logger = get_logger("investigation_evidence_service")

class InvestigationEvidenceService:
    def __init__(self, session: AsyncSession):
        self.session = session
        self.repo = InvestigationEvidenceRepository(session)

    async def list_items(self, params: InvestigationEvidenceFilterParams) -> Tuple[Sequence[InvestigationEvidence], int]:
        skip = (params.page - 1) * params.page_size
        logger.info("Listing InvestigationEvidence items", page=params.page, page_size=params.page_size)
    ...
    async def get_by_id(self, item_id: str) -> InvestigationEvidence:
        entity = await self.repo.get_by_id(item_id)
        if not entity:
            logger.warning("InvestigationEvidence not found", item_id=item_id)
            raise NotFoundError(f"InvestigationEvidence with ID '{item_id}' was not found")
        return entity

    async def create(self, req: InvestigationEvidenceCreate) -> InvestigationEvidence:
        ...
        logger.info("InvestigationEvidence created successfully", item_id=getattr(entity, "id", None))
        return entity

    async def update(self, item_id: str, req: InvestigationEvidenceUpdate) -> InvestigationEvidence:
        ...
        logger.info("InvestigationEvidence updated successfully", item_id=item_id)
        return entity

    async def delete(self, item_id: str) -> None:
        entity = await self.get_by_id(item_id)
        await self.repo.delete(entity)
        logger.info("InvestigationEvidence deleted successfully", item_id=item_id)
