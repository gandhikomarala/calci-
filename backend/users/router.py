"""FastAPI REST API Router for User (User management, credential lifecycle, and analyst profiles)."""
...
async def create_item(req: UserCreate, session: AsyncSession = Depends(get_db_session)):
    service = UserService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=UserResponse)
async def update_item(item_id: str, req: UserUpdate, session: AsyncSession = Depends(get_db_session)):
    service = UserService(session)
