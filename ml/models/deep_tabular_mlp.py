"""DeepTabularMLPModel: Multi-layer perceptron neural network for tabular customer churn."""
...
class DeepTabularMLPModel(BaseChurnModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "DeepTabularMLPModel":
        logger.info("Fitting DeepTabularMLPModel", samples=len(X), features=len(X.columns))
        self.model.fit(X, y)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("DeepTabularMLPModel must be trained before calling predict_proba()")
