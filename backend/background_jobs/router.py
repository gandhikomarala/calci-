"""FastAPI REST API Router for BackgroundJob (Asynchronous Celery task execution status and progress tracking)."""
...
router = APIRouter(prefix="/background_jobs", tags=["BackgroundJob"])

@router.get("", response_model=PaginatedResponse[BackgroundJobResponse])
async def list_items(
    ...
    params = BackgroundJobFilterParams(
        page=page,
        page_size=page_size,
        search=search,
        is_active=is_active,
...
@router.get("/{item_id}", response_model=BackgroundJobResponse)
...
async def create_item(req: BackgroundJobCreate, session: AsyncSession = Depends(get_db_session)):
    service = BackgroundJobService(session)
    return await service.create(req)

@router.put("/{item_id}", response_model=BackgroundJobResponse)
async def update_item(item_id: str, req: BackgroundJobUpdate, session: AsyncSession = Depends(get_db_session)):
    service = BackgroundJobService(session)
