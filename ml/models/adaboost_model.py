"""AdaBoostChurnModel: Adaptive Boosting sequential decision stump classifier."""
...
class AdaBoostChurnModel(BaseChurnModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "AdaBoostChurnModel":
        logger.info("Fitting AdaBoostChurnModel", samples=len(X), features=len(X.columns))
        self.model.fit(X, y)
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_trained:
            raise RuntimeError("AdaBoostChurnModel must be trained before calling predict_proba()")
