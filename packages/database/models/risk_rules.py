"""SQLAlchemy 2.0 Mapped Entity Model for RiskRule (Configurable fraud detection business rules and conditional predicates)."""
...
class RiskRule(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_risk_rules_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<RiskRule(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
