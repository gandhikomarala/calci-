"""SQLAlchemy 2.0 Mapped Entity Model for ModelVersion (Immutable model binary versions, artifact hashes, and metrics)."""
...
class ModelVersion(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index("ix_model_versions_created_active", "created_at", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<ModelVersion(id='{self.id}', code='{self.code}', is_active={self.is_active})>"
